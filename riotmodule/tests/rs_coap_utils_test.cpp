...
TEST(rs_coap_utils, simple_query) {
    ...
    split_coap_query(query.length(), cquery, &result);
...
TEST(rs_coap_utils, str_index_of) {
    ASSERT_EQ(str_index_of("key=value", '='), 3);
    ASSERT_EQ(str_index_of("longkey=value", '='), 7);
    ASSERT_EQ(str_index_of("=value", '='), 0);
    ASSERT_EQ(str_index_of("value", '='), -1);
    ASSERT_EQ(str_index_of("", '='), -1);
}